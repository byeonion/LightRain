...
from .forms import LoginForm, ArticleEditForm, PostForm, ListForm, ConfirmForm
...
def article_edit(id):
    ...
    if form.validate_on_submit():
        ...
        article.body = form.body.data #.replace('\r\n','<br>')
        article.list = List.query.get(form.list.data)
        article.lastedit = datetime.utcnow()
    ...
    form.list.data = article.role_id
...
def list(id):
    ...
    return render_template('list.html', list=list, id=list.id, articles=articles)
...
# def list_del(id):
    ...
    # db.session.commit()
    # flash('已删除,该目录文章已放入"未分类"')
    # lists = List.query.order_by(List.id).all()
    # articles = Article.query.all()
    # for article in articles:
        # if article.list is None:
            # article.list = List.query.filter_by(listname='未分类').first()
            # db.session.add(article)
        # db.session.commit()
...
@main.route('/confirm/<int:id>', methods=['GET', 'POST'])
@login_required
def confirm(id):
    form = ConfirmForm()
    article = Article.query.get_or_404(id)
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('main.index'))
    return render_template('confirm.html', form=form, article=article)
    
    
@main.route('/confirm_list/<int:id>', methods=['GET', 'POST'])
@login_required
def confirmList(id):
    form = ConfirmForm()
    list = List.query.get_or_404(id)
    ...
    return render_template('confirm_list.html', form=form, list=list)
